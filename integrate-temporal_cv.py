...
logi_l1_grid_search = GridSearchCV(LogisticRegression(max_iter=100000,penalty='l1'), param_grid,cv=temporal_cv)
logi_l1_grid_search.fit(data_all_x, data_all_y,groups=groups)
...
logi_l2_grid_search = GridSearchCV(LogisticRegression(max_iter=100000,penalty='l2'), param_grid,cv=temporal_cv)
logi_l2_grid_search.fit(data_all_x, data_all_y,groups=groups)
print('best cv accuracy')
...
decision_tree_grid_search = GridSearchCV(tree.DecisionTreeClassifier(), param_grid, cv=temporal_cv)
decision_tree_grid_search.fit(data_all_x, data_all_y,groups=groups)
...
random_forest_grid_search = GridSearchCV(RandomForestClassifier(), param_grid, cv=temporal_cv)
random_forest_grid_search.fit(data_all_x, data_all_y,groups=groups)
...
clever_print('KNN')
knn=KNeighborsClassifier().fit(data_train_x,data_train_y)
print('accuracy on training')
print(accuracy_score(data_train_y,knn.predict(data_train_x)))
print('accuracy on dev')
print(accuracy_score(data_dev_y,knn.predict(data_dev_x)))


to_cv_nei = range(1,100,5)
param_grid = {'n_neighbors':to_cv_nei}

knn_cv= GridSearchCV(KNeighborsClassifier(), param_grid, cv=temporal_cv)
knn_cv.fit(data_all_x, data_all_y,groups=groups)
...
param_grid = {'C':to_cv_c}
svm_linear_cv= GridSearchCV(SVC(kernel='linear'), param_grid, cv=temporal_cv)
svm_linear_cv.fit(svm_scaler.transform(data_all_x), data_all_y,groups=groups)
...
svm_rbf_cv= GridSearchCV(SVC(kernel='rbf'), param_grid, cv=temporal_cv)
svm_rbf_cv.fit(svm_scaler.transform(data_all_x), data_all_y,groups=groups)
...
mlp_cv= GridSearchCV(MLPClassifier(max_iter=10000), param_grid, cv=temporal_cv)
mlp_cv.fit(nn_scaler.transform(data_all_x), data_all_y,groups=groups)
mlp_cv.best_params_
