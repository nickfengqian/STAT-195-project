...
df=pd.concat([df,pd.get_dummies(df.Poll_Rating,prefix='Poll_Rating')],axis=1)
...
df['State'] = df['Orig_District_ID'].map(getTwo)
df['State'].nunique()

dist_cols = []
for col in df.columns:
    if 'District_ID_' in col:
        dist_cols += [col]
...
train_columns=['PVI', 'Poll_Rating_1', 'Poll_Rating_2', 'MISSING_minority_percentage', 'minority_percentage', 'Poll_Rating_8', 'MISSING_Raised', 'Poll_Rating_7', 'Poll_Rating_6', 'State_MN', 'State_MP', 'MISSING_Presidential_Approval_Rating', 'State_KS', 'Gender_0.0', 'Poll_Rating_4', 'State_MS', 'State_OR', 'State_MA', 'State_RI', 'State_VA', 'State_OH', 'State_WY', 'State_DE', 'State_MT', 'State_VI', 'MISSING_seat_transition']
...
knn_cv.fit(data_all_x, data_all_y)
